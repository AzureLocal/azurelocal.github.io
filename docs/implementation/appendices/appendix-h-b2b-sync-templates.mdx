...
This appendix contains JSON template files for configuring Entra ID B2B Cross-Tenant Synchronization (CTS) between Azure Local Cloud MGMT and customer tenants.

These templates implement the **Azure Local Cloud approach** which provisions users as **Member** type (not Guest), enabling:
...
| `Azure Local Cloud-outbound-policy.json` | Cross-tenant access policy for outbound sync | Azure Local Cloud MGMT |
| `cts-application.json` | CTS application instantiation template | Azure Local Cloud MGMT |
| `cts-credentials.json` | Target tenant credentials configuration | Azure Local Cloud MGMT |
| `cts-provisioning-job.json` | Provisioning job with attribute mappings | Azure Local Cloud MGMT |
| `cts-group-assignment.json` | Group assignment to CTS configuration | Azure Local Cloud MGMT |
...
| `{{Azure Local Cloud_MGMT_TENANT_ID}}` | Azure Local Cloud MGMT tenant ID | `00000000-aaaa-bbbb-cccc-111111111111` |
...
### Azure Local Cloud Service Groups

| Variable | Group Name | Description |
|----------|------------|-------------|
| `{{GROUP_ID_PUBLIC_CLOUD_SERVICES}}` | Public Cloud Services | Core cloud services team |
| `{{GROUP_ID_PUBLIC_CLOUD_SUPPORT}}` | Public Cloud Support | Support team |
| `{{GROUP_ID_PUBLIC_CLOUD_IMPLEMENTATION}}` | Public Cloud Implementation Engineering | Implementation engineers |
| `{{GROUP_ID_PUBLIC_CLOUD_PREMIUM}}` | Public Cloud Premium | Premium tier support |
| `{{GROUP_ID_SOC}}` | SOC | Security operations center |
| `{{GROUP_ID_ADVANCED_SERVICES}}` | SEC-SD Advanced Services | Advanced enterprise services |
...
This policy is deployed to the **Customer Tenant** to allow inbound CTS from Azure Local Cloud.

```json
{
 "tenantId": "{{Azure Local Cloud_MGMT_TENANT_ID}}",
...
## Azure Local Cloud Outbound Policy

This policy is deployed to **Azure Local Cloud MGMT** to allow outbound sync to customer tenants.
...
```json
{
 ...
 "targets": [
 {
 "target": "{{GROUP_ID_PUBLIC_CLOUD_SERVICES}}",
 "targetType": "group"
 },
 {
 "target": "{{GROUP_ID_PUBLIC_CLOUD_SUPPORT}}",
 "targetType": "group"
 },
 {
 "target": "{{GROUP_ID_PUBLIC_CLOUD_IMPLEMENTATION}}",
 "targetType": "group"
 },
 {
 "target": "{{GROUP_ID_PUBLIC_CLOUD_PREMIUM}}",
 "targetType": "group"
 },
 {
 "target": "{{GROUP_ID_SOC}}",
 ...
 {
 "target": "{{GROUP_ID_ADVANCED_SERVICES}}",
 "targetType": "group"
 }
 ]
 },
...
Creates the Cross-Tenant Synchronization application in Azure Local Cloud MGMT.

```json
{
 "displayName": "Azure Local Cloud CTS to {{CUSTOMER_NAME}}",
 "description": "Cross-Tenant Synchronization application for automated user provisioning from Azure Local Cloud MGMT to {{CUSTOMER_NAME}}",
...
```json
{
 ...
 "description": "Cross-Tenant Synchronization provisioning job from Azure Local Cloud MGMT to {{CUSTOMER_NAME}}",
 "enabled": true,
 "sourceIdentity": {
 "type": "AzureActiveDirectory",
 "id": "{{Azure Local Cloud_MGMT_TENANT_ID}}"
 },
 "targetIdentity": {
 "type": "AzureActiveDirectory",
