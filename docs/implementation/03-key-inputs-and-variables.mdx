...
sidebar_position: 3
description: "Configuration management system for Azure Local Anywhere deployments using master-registry.yaml and infrastructure.yml."
category: "RUNBOOK"
...
Azure Local Anywhere deployments use a **standardized configuration management system** based on two core files:

1. **master-registry.yaml** - Variable DEFINITIONS (names, types, constraints, validation rules)
2. **infrastructure.yml** - Environment-specific VALUES (actual configuration data)
...
The `infrastructure.yml` file is populated during the planning and discovery phase. Key planning documents provide the source values:
...
4. Populate `infrastructure.yml` with discovered values
5. Validate against `master-registry.yaml` constraints
...
└─────────────────────────────────────────────────────────────────┘
 ...
 │
┌─────────────────────────────────────────────────────────────────┐
│ MASTER VARIABLE REGISTRY (Definitions) │
│ Defines: Variable names, types, constraints, patterns │
│ Version: 2.1.0 (910+ variables) │
└─────────────────────────────────────────────────────────────────┘
 │
 │ Validates ↓
 │
┌─────────────────────────────────────────────────────────────────┐
│ infrastructure.yml (Environment Values) │
...
## How Scripts Use infrastructure.yml

All deployment and automation scripts read configuration from `infrastructure.yml` using **fixed variable paths**. This ensures scripts are environment-agnostic and portable across all deployments.
...
3. **Read values using fixed paths**
 ```powershell
 # These paths are guaranteed to exist in every infrastructure.yml
...
These paths are standardized across all `infrastructure.yml` files:
...
- Only the values in `infrastructure.yml` differ—script logic stays the same
...
Values wrapped in `{{...}}` (e.g., `{{platform.kv_platform_name}}`) are **variable path references** from your `infrastructure.yml`. Replace them with the actual values from your deployment configuration. For example, using the fictional **Infinite azurelocal Corp**:
...
### 2. master-registry.yaml

**Location:** Copied to your environment repository's `variables/` folder

**Purpose:** Defines all variable names, types, constraints, and validation rules

**You typically don't edit this file** - it's the source of truth maintained centrally and copied to each environment for validation purposes.

---

## Key Planning Inputs

The following table shows how planning phase outputs map to `infrastructure.yml` variables:
...
**ALWAYS validate your infrastructure.yml before deployment:**

```powershell
# Validate configuration against schema
.\tools\Test-Infrastructure.ps1 -ConfigFile .\infrastructure.yml
```
...
```
✅ Validation successful: infrastructure.yml conforms to schema
...
```
**Fix:** Add missing variable to infrastructure.yml

---

## Secrets Management

:::danger NEVER commit secrets to infrastructure.yml
...
```powershell
# Load configuration
$config = Get-Content -Path ".\infrastructure.yml" -Raw | ConvertFrom-Yaml
...
# Load configuration
locals {
 config = yamldecode(file("infrastructure.yml"))
}
...
param configFile string = 'infrastructure.yml'
...
After gathering all required inputs and creating your infrastructure.yml:
...
- Version: 2.0.0
- Tags: azure-local, configuration, variables, infrastructure-yml
- Keywords: key inputs, variables, infrastructure.yml, master-registry.yaml
- Author: Azure Local Cloudnology Team
