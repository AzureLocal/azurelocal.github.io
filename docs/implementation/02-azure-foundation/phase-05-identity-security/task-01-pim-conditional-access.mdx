...
> **When to use**: Scripted PIM configuration from management workstation — reads values from `infrastructure.yml`

:::note No Toolkit Script
There is no dedicated toolkit script for PIM configuration. Use the Microsoft Graph PowerShell commands below. The commands read subscription and user IDs from `infrastructure.yml`.
:::
...
> **When to use**: Copy-paste ready — no `infrastructure.yml`, no helpers, no dependencies. Edit the `#region CONFIGURATION` block and run.
...
> **When to use**: Scripted CA policy creation from management workstation — reads break-glass account IDs from `infrastructure.yml`
...
> **When to use**: Copy-paste ready — no `infrastructure.yml`, no helpers, no dependencies. Edit the `#region CONFIGURATION` block with your break-glass account object IDs.
...
- [ ] Object IDs documented in `infrastructure.yml` under `identity.break_glass_account_ids`
