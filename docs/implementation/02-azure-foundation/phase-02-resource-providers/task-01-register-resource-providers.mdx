...
| **`infrastructure.yml`** | Configured with target subscription ID |
...
1. **Navigate to Resource Providers**:
 ...
 - Select your target subscription (`infrastructure.yml → azure.subscriptions.lab.id`)
...
This script is completely self-contained. Edit the `$SubscriptionId` in the `#region CONFIGURATION` block and run — no `infrastructure.yml`, no config-loader, no helpers required.
:::
