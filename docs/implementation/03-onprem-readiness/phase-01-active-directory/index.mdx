...
Create 7 security groups for role-based access control of Azure Local operations. Group names follow the convention `SG-{org_prefix}-{cluster_id}-AZL-{role}`, driven by `active_directory.security_groups.org_prefix` and `cluster_id` in `infrastructure.yml`.
