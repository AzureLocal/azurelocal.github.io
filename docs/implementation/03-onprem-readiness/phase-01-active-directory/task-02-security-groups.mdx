...
updated: 2026-03-03
version: 1.1
tags: ["azure-local", "active-directory", "security-groups", "rbac"]
keywords: ["security groups", "AD groups", "RBAC", "role-based access"]
status: Active
---
...
Group names follow the convention `SG-{org_prefix}-{cluster_id}-AZL-{role}`, built dynamically from two fields in `active_directory.security_groups` in `infrastructure.yml`. The `cluster_id` suffix makes groups unique per cluster in the same domain.
