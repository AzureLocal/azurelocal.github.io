...
If cluster nodes cannot reach Azure endpoints directly, deploy an [Arc Gateway](https://learn.microsoft.com/en-us/azure/azure-arc/servers/arc-gateway) to proxy Arc traffic. Arc Gateway configuration is defined in `infrastructure.yml` under `deployment.arc_gateway`.
