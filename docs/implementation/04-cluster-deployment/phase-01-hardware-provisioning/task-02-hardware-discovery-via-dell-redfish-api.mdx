...
updated: 2026-03-04
version: 1.2
tags: ["azure-local", "hardware-discovery", "dell", "redfish", "idrac", "inventory"]
keywords: ["hardware discovery", "Dell Redfish API", "iDRAC", "hardware inventory", "BIOS", "service tag", "MAC address"]
status: Active
---
...
|-------------|---------|--------|
| iDRAC IPs | All nodes reachable on port 443 | `nodes.<name>.idrac_ip` in `infrastructure.yml` |
| iDRAC Credentials | Admin username and password | Key Vault or `infrastructure.yml` |
...
After collecting data from all nodes, manually update the following fields in `infrastructure.yml`:
...
:::warning Save and commit infrastructure.yml after updating
All subsequent tasks depend on these values. Do not proceed to Task 03 until `infrastructure.yml` is updated and committed.
:::

</TabItem>
<TabItem value="orchestrated" label="Orchestrated Script (Mgmt Server)">

> **When to use**: Standard deployment — runs from the management server, reads node list from `infrastructure.yml`
...
.DESCRIPTION
 ...
 to write discovered values back into infrastructure.yml.
...
function Resolve-ConfigPath {
 ...
 $defaultIndex = ($candidates | ForEach-Object { $_.Name }).IndexOf("infrastructure.yml")
...
try {
 ...
 # Update infrastructure.yml unless skipped
 if (-not $SkipYmlUpdate) {
 Write-Log ""
 Write-Log "=== Updating infrastructure.yml from discovery data ===" "HEADER"
 ...
 Write-Log "Skipping infrastructure.yml update (-SkipYmlUpdate specified)." "WARN"
...
1. Detect your `infrastructure.yml` (or prompt if multiple `infrastructure*.yml` files are found)
...
After the script completes, two options to update `infrastructure.yml`:

**Option A — Manual update**

Open `infrastructure.yml` and fill in the following fields using values from the JSON files in `configs/network-devices/bmc/`:
...
The script will auto-detect your `infrastructure.yml`, show a diff of all changes, and prompt for confirmation before writing. A timestamped backup is created automatically.

:::warning Do not proceed to Task 03 until infrastructure.yml is updated
...
| 1.2 | 2026-03-04 | Azure Local Cloud Azure Local Cloudnology | Add infrastructure.yml update instructions to all three tabs; document Update-InfrastructureYml-FromDiscovery.ps1 integration |