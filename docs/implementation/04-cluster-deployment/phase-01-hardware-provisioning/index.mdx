...
Network VLAN assignments are site-specific and defined in `infrastructure.yml`.
:::
...
Test iDRAC connectivity for all nodes before starting. iDRAC IPs are defined in `infrastructure.yml` under `nodes.<name>.idrac_ip`.

```powershell
# Load iDRAC IPs from infrastructure.yml
