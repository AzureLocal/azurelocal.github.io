...
sidebar_position: 1
description: "Create DHCP reservations for iDRAC out-of-band management interfaces using MAC addresses and planned IPs from infrastructure.yml before hardware arrives on site."
category: Runbook
...
updated: 2026-03-04
version: 1.1
tags: ["azure-local", "dhcp", "idrac", "hardware-provisioning", "fortigate", "windows-dhcp"]
keywords: ["DHCP reservation", "iDRAC IP", "out-of-band management", "FortiGate DHCP", "Windows DHCP", "MAC address"]
status: Active
---
...
Create DHCP reservations for iDRAC out-of-band management interfaces using MAC addresses and planned IPs from `infrastructure.yml`. This task is performed **before hardware arrives on site** — MAC addresses come from the Dell order documentation (packing slip or TechDirect), not from a discovery scan.
...
| OOB Network Subnet | Out-of-band VLAN and DHCP scope configured | `infrastructure.yml: network.vlans.oob.cidr` |
| Node MAC Addresses | iDRAC MAC addresses from Dell order documentation | `infrastructure.yml: nodes.<name>.macs.idrac` |
| Planned iDRAC IPs | Assigned iDRAC IP per node | `infrastructure.yml: nodes.<name>.idrac_ip` |
...
4. In the **Reserved Addresses** section, click **Create New** for each node:
 - **IP Address**: `nodes.<name>.idrac_ip` from `infrastructure.yml`
 - **MAC Address**: `nodes.<name>.macs.idrac` from `infrastructure.yml`
...
- [ ] IP addresses match `infrastructure.yml: nodes.<name>.idrac_ip`
- [ ] MAC addresses match `infrastructure.yml: nodes.<name>.macs.idrac`
...
- [ ] IP addresses match `nodes.<name>.idrac_ip` in `infrastructure.yml`
- [ ] MAC addresses match `nodes.<name>.macs.idrac` in `infrastructure.yml`
- [ ] Reservation names follow convention: `<hostname>-iDRAC`
