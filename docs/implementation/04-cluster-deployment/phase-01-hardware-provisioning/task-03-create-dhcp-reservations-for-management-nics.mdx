...
sidebar_position: 3
description: "Create DHCP reservations for in-band management network interfaces using MAC addresses from Task 02 hardware discovery and planned IPs from infrastructure.yml."
category: Runbook
...
Create DHCP reservations for in-band management NICs using MAC addresses from `infrastructure.yml` (populated from Task 02 hardware discovery) and planned IPs from `infrastructure.yml`. Although these interfaces are ultimately assigned **static IPs during OS configuration**, DHCP reservations provide consistent temporary addressing for remote automation workflows.
...
|-------------|-------------|--------|
| Task 02 Complete | Management NIC MACs populated in `infrastructure.yml` | `infrastructure.yml: nodes.<name>.macs.onboard_port1` |
| DHCP Server Access | FortiGate admin rights, Windows DHCP admin, or customer DHCP contact | Network team |
| Management Network Subnet | Management VLAN DHCP scope configured | `infrastructure.yml: network.vlans.management.cidr` |
| Planned Management IPs | Assigned management IP per node | `infrastructure.yml: nodes.<name>.management_ip` |

---

## Variables from infrastructure.yml
...
4. In the **Reserved Addresses** section, click **Create New** for each node:
 - **IP Address**: `nodes.<name>.management_ip` from `infrastructure.yml`
 - **MAC Address**: `nodes.<name>.macs.onboard_port1` from `infrastructure.yml`
 - **Description**: `<hostname>-mgmt (Service Tag: <service-tag>)`
5. Click **OK** to save

### FortiOS CLI (from Management Server)

Connect via SSH from the management server and run the following for each node. Replace values with actual entries from `infrastructure.yml`.
...
Extract and provide the following from `infrastructure.yml`:

| Field | infrastructure.yml Path | Example |
|-------|------------------------|---------|
...
1. Extract node data from `infrastructure.yml`
...
- [ ] IP addresses match `infrastructure.yml: nodes.<name>.management_ip`
- [ ] MAC addresses match `infrastructure.yml: nodes.<name>.macs.onboard_port1`
...
- [ ] IP addresses match `nodes.<name>.management_ip` in `infrastructure.yml`
- [ ] MAC addresses match `nodes.<name>.macs.onboard_port1` in `infrastructure.yml`
- [ ] Reservation names follow convention: `<hostname>-mgmt`
