...
updated: 2026-03-10
version: 1.0
tags: ["azure-local", "security-groups", "active-directory", "rbac", "local-groups", "post-deployment"]
keywords: ["Add-LocalGroupMember", "Administrators", "Remote Management Users", "security group", "cluster node", "lusrmgr"]
status: Active
---
...
**Status**: Active
**Applies To**: All cluster nodes following Phase 05 cluster deployment
...
> **When to use**: Config-driven automation from a management server or jump box — reads `infrastructure.yml`, iterates all cluster nodes via PSRemoting
...
# Reads all values — group assignments driven by local_groups in infrastructure.yml
...
> **When to use**: Copy-paste ready — no `infrastructure.yml`, no helpers, no external dependencies. Run from a management server or jump box. All values are defined in the `#region CONFIGURATION` block.
