...
- `marketplace_images` block populated in `infrastructure.yml`
...
`infrastructure.yml` and creates all images in sequence. Run from the management server.
...
**When to use:** When deploying without `infrastructure.yml` — external sharing, one-off
