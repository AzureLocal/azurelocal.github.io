...
The SDN deployment requires the external vSwitch name to be populated at `compute.azure_local.vm_switch_name` in `infrastructure.yml` before proceeding.
...
Copy the highlighted `vm_switch_name` value from the output into `configs/infrastructure.yml` on the management server:

```yaml title="configs/infrastructure.yml"
compute:
 azure_local:
...
- [ ] `compute.azure_local.vm_switch_name` is populated in `infrastructure.yml`

</TabItem>
<TabItem value="orchestrated" label="Orchestrated Script (Mgmt Server)">

> **When to use**: Managing from management server or jump box — config-driven via `infrastructure.yml` and PSRemoting
...
All values are defined in the `#region CONFIGURATION` block at the top. No `infrastructure.yml`, no config-loader, no helpers required. Run directly on the domain controller via RDP or console.
:::
...
:::info No External Dependencies
All values are defined in the `#region CONFIGURATION` block at the top. No `infrastructure.yml`, no config-loader, no helpers required. Requires PSRemoting access to a cluster node with the Azure Stack HCI Administrator role.
:::
...
| vSwitch name set | `infrastructure.yml` → `compute.azure_local.vm_switch_name` | Non-empty string |
...
- [ ] `compute.azure_local.vm_switch_name` populated in `infrastructure.yml`
...
*Part of the [Azure Local Anywhere Implementation Guide](../../index.mdx).*
