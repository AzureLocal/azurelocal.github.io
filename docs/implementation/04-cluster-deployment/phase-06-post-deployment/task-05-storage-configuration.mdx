...
the **Discovery & Planning** phase. If those values are not in your `infrastructure.yml`,
...
Once you have the outputs, populate the following keys in your `infrastructure.yml` before
...
`cluster_shared_volumes` block directly into your `infrastructure.yml`. Skip if the YAML
...
**What it does:**
- Reads your existing `infrastructure.yml` and pre-fills any values already present
...
Open `infrastructure.yml` after the script completes and verify the `cluster_shared_volumes`
