...
updated: 2026-03-04
version: 1.1
tags: ["azure-local", "os-installation", "azure-stack-hci", "verification", "idrac", "powershell"]
keywords: ["Azure Stack HCI OS", "OS verification", "build number", "BOSS card", "Server Core", "SConfig", "Test-OsDeployment"]
status: Active
---
...
| iDRAC console access | Virtual Console accessible for each node | `infrastructure.yml: nodes.<name>.idrac_ip` |
...
Run the toolkit verification script from the management server. It reads all node details from `infrastructure.yml`, connects via PowerShell remoting, and produces a summary report.
