...
| iDRAC Access | Virtual Console and Virtual Media control available | `infrastructure.yml: nodes.<name>.idrac_ip` |
...
Verify iDRAC Virtual Console and Virtual Media access for all nodes before starting. iDRAC IPs are defined in `infrastructure.yml` under `nodes.<name>.idrac_ip`.
