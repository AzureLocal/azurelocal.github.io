...
updated: 2026-03-04
version: 1.1
tags: ["azure-local", "os-installation", "azure-stack-hci", "idrac", "virtual-console", "windows-setup", "boss"]
keywords: ["Azure Stack HCI OS", "Windows Setup", "iDRAC Virtual Console", "BOSS card", "manual OS installation", "Server Core"]
status: Active
---
...
| iDRAC access | Virtual Console access to each node | `infrastructure.yml: nodes.<name>.idrac_ip` |
...
Record the Administrator password and store it in the Key Vault referenced in `infrastructure.yml` under the `key_vault.name` value (secret name: `node-<hostname>-local-admin`). Do not save it in any file or document.
:::
