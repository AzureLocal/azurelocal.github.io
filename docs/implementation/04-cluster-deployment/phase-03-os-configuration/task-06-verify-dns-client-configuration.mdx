...
purpose: Confirm DNS configuration matches infrastructure.yml and that Azure endpoints resolve from each node
author: Azure Local Cloud Azure Local Cloudnology
created: 2026-01-31
updated: 2026-03-04
version: 2.0
tags: ["azure-local", "os-configuration", "networking", "dns", "validation", "powershell"]
keywords: ["verify DNS", "Test-DnsClientConfig", "Invoke-VerifyDNS-Orchestrated", "dns.primary", "dns.secondary", "Resolve-DnsName"]
status: Active
---
...
> **SCOPE**: DNS client verification 
> **PURPOSE**: Confirm DNS configuration matches `infrastructure.yml` and that Azure endpoints 
...
3. Read the configured DNS servers and compare against expected values from `infrastructure.yml`
...
| DNS IPs confirmed | Validated DNS server addresses | `infrastructure.yml`: `dns.primary`, `dns.secondary` |
| NIC name confirmed | Management adapter name | `infrastructure.yml`: `cluster.management_nic_name` |
...
| `infrastructure.yml` path | Script variable | Example |
|----------------------------|----------------|---------|
...
| Variable | `infrastructure.yml` path | Example |
|----------|----------------------------|---------|
...
`nodes.<name>.management_ip` values from `infrastructure.yml`. Connects to each
...
if (-not $mgmtNIC) { throw "cluster.management_nic_name not found in infrastructure.yml" }
if (-not $expectedPrimary) { throw "dns.primary not found in infrastructure.yml" }
if (-not $expectedSecondary){ throw "dns.secondary not found in infrastructure.yml" }
if ($nodeNames.Count -eq 0) { throw "No nodes found in infrastructure.yml" }
...
foreach ($nodeName in $nodeNames) {
 ...
 if (-not $mgmtIP) {
 Write-Log "[$nodeName] management_ip not found in infrastructure.yml - skipping" "WARN"
 continue
 }
...
[2026-03-04 10:00:00] [INFO ] Config: C:\configs\infrastructure.yml
...
- [ ] Primary DNS server matches `infrastructure.yml`: `dns.primary`
- [ ] Secondary DNS server matches `infrastructure.yml`: `dns.secondary`
...
|-------|------------|-------------|
| Script hard-fails on startup | `REPLACE` placeholder values remain | Edit `#region CONFIGURATION` with real values from `infrastructure.yml` |
| `Adapter not found` error | NIC name in config doesn't match | Run `Get-NetAdapter` on the node; update `cluster.management_nic_name` in `infrastructure.yml` |
| DNS mismatch — wrong IPs returned | Task 05 not completed or used incorrect values | Re-run Task 05 with correct values from `infrastructure.yml` |
...
| Orchestrated script: `cluster.management_nic_name not found` | Key missing from `infrastructure.yml` | Add `management_nic_name` to the `cluster:` block in yml |
| Orchestrated script: `dns.primary not found` | Key missing from `infrastructure.yml` | Add `dns.primary` and `dns.secondary` to the `dns:` block in yml |
