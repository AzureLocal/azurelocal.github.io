---
title: "Task 16: Phase 03 Verification"
sidebar_label: "Task 16: Verification"
sidebar_position: 16
description: "Read-only verification of all Phase 03 OS Configuration tasks (01–15) across all cluster nodes"
category: "Runbook"
scope: "Read-only verification of all Phase 03 OS Configuration tasks (01–15) across ..."
purpose: "Read-only verification of all Phase 03 OS Configuration tasks (01–15) across all cluster nodes"
...
Read-only verification script that confirms all Phase 03 OS Configuration tasks completed successfully across all cluster nodes. Run this after [Task 15](./task-15-complete-combined-script-all-steps.mdx) completes — or after running individual tasks — before proceeding to Phase 04 ARC Registration.
...
| **12** | FC HBA Drivers (SAN only) | FC initiator ports visible via `Get-InitiatorPort` |
| **13** | MPIO/MSDSM (SAN only) | MPIO feature installed; vendor hardware IDs registered |
| **14** | LUN Presentation (SAN only) | Infrastructure and perf-history LUNs RAW, all nodes, correct path count |
| **15** | Combined Script | Nodes restarted, all baseline config applied |
...
| Requirement | Details |
|---|---|
| Phase 03 Tasks 01–15 complete | All OS configuration tasks run |
...
```powershell
# Task 16 - Phase 03 Verification (all nodes)
.\scripts\deploy\04-cluster-deployment\phase-03-os-configuration\task-16-phase03-verification\powershell\Invoke-Phase03Verification-Orchestrated.ps1 `
...
```powershell
# Task 16 - Phase 03 Verification (direct, run on each node)
...
scripts/deploy/04-cluster-deployment/phase-03-os-configuration/
 task-16-phase03-verification/powershell/
...
[← Task 15: Combined Script](./task-15-complete-combined-script-all-steps.mdx) · [↑ Phase 03](./index.mdx) · [Phase 04: ARC Registration →](../phase-04-arc-registration/index.mdx)
