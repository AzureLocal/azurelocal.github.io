...
sidebar_position: 5
description: "Configure primary and secondary DNS server addresses on the management NIC of each Azure Local cluster node using explicit values from infrastructure.yml."
category: Runbook
scope: DNS client configuration
purpose: Set DNS server addresses on the management NIC of all nodes using explicit values from infrastructure.yml
...
> from `infrastructure.yml` — adapter resolved by exact name, not auto-detected 
...
5. Enter the **Primary DNS IP** from `infrastructure.yml` → `dns.primary`
6. Enter the **Secondary DNS IP** from `infrastructure.yml` → `dns.secondary`
7. Press Enter to confirm

**Verify in SConfig:**

After setting DNS, return to the Network settings menu and confirm the DNS servers shown
match the values from `infrastructure.yml`.
...
Run from the management server. Reads all DNS and NIC values from `infrastructure.yml` — no manual variable editing required.
...
**`infrastructure.yml` values used:**
...
[2026-03-05 09:22:00] [INFO] Loading configuration from: C:\configs\infrastructure.yml
...
|-------|----------|--------|
| Primary DNS on management NIC | `dns.primary` from `infrastructure.yml` | ☐ |
| Secondary DNS on management NIC | `dns.secondary` from `infrastructure.yml` | ☐ |
...
| Orchestrated script: `cluster.management_nic_name not found` | Key missing from `infrastructure.yml` | Add `cluster.management_nic_name` to the `cluster:` block in yml |
...
| 2.0 | 2026-03-05 | Azure Local Cloud Azure Local Cloudnology | Full rewrite to standards — complete frontmatter, 3-tab structure (SConfig, Direct, Orchestrated), Standalone tab removed, full embedded scripts, explicit NIC name from infrastructure.yml, infrastructure.yml integration in orchestrated script |
