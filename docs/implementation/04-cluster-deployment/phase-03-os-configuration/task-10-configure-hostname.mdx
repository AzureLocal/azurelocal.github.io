...
Rename each node to its target hostname from `infrastructure.yml`. Nodes must restart to apply the change.
...
| Hostnames from infrastructure.yml | `cluster_nodes[].hostname` |
...
3. Type the hostname from `infrastructure.yml` (`cluster_nodes[].hostname`)
...
Run from the management server. Reads all node IPs and hostnames from `infrastructure.yml`, renames each node, restarts it, and waits for it to come back online.
...
$ConfigPath = "$env:USERPROFILE\infrastructure.yml"
...
- [ ] Hostnames match `cluster_nodes[].hostname` in infrastructure.yml
