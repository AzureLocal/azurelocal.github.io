...
purpose: Configure w32tm with the NTP server from infrastructure.yml so all cluster nodes synchronize time from the same authoritative source
author: Azure Local Cloud Azure Local Cloudnology
created: 2026-01-31
updated: 2026-03-04
version: 2.0
tags: ["azure-local", "os-configuration", "ntp", "time-sync", "w32tm", "powershell"]
keywords: ["configure NTP", "Set-NTPConfiguration", "Invoke-ConfigureNTP-Orchestrated", "active_directory.ntp_servers", "w32tm", "Windows Time service"]
status: Active
---
...
> **SCOPE**: Time synchronization 
> **PURPOSE**: Configure w32tm with the NTP server from `infrastructure.yml` so all cluster nodes synchronize to the same authoritative time source — required for Kerberos, Active Directory, and Arc registration 
...
| NTP server address | FQDN or IP of the authoritative NTP server | `infrastructure.yml`: `active_directory.ntp_servers` |
...
:::info infrastructure.yml — NTP server list
...
Set `$NTPServer` to the first entry from `active_directory.ntp_servers` in `infrastructure.yml`.
...
`infrastructure.yml`. Connects to each node via PSRemoting and configures w32tm.
...
.EXAMPLE
 .\Invoke-ConfigureNTP-Orchestrated.ps1
 .\Invoke-ConfigureNTP-Orchestrated.ps1 -ConfigPath "C:\configs\infrastructure.yml"
#>
...
if ($ntpList.Count -eq 0) { throw "active_directory.ntp_servers not found or empty in infrastructure.yml" }
if ($nodes.Count -eq 0) { throw "cluster_nodes not found or empty in infrastructure.yml" }
...
|-------|------------|-------------|
| Script hard-fails on startup | `REPLACE` placeholder value remains | Edit `#region CONFIGURATION` with the NTP server from `infrastructure.yml` |
...
| Orchestrated: `ntp_servers not found` | Key missing or wrong path in `infrastructure.yml` | Confirm `active_directory.ntp_servers:` list exists in yml |
