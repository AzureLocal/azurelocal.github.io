...
updated: 2026-03-04
version: 2.0
tags: ["azure-local", "os-configuration", "rdp", "remote-desktop", "powershell"]
keywords: ["RDP", "Remote Desktop", "fDenyTSConnections", "SConfig", "Enable-NetFirewallRule"]
status: Active
---
...
| iDRAC console access | Virtual Console accessible for each node (if using SConfig or Direct tab) | `infrastructure.yml: nodes.<name>.idrac_ip` |
...
if ($rdp.fDenyTSConnections -eq 0) {
 ...
 Write-Host "RDP not enabled" -ForegroundColor Red
}
```
...
Run from the management server. Reads node names from `infrastructure.yml`.
...
| 2.0 | 2026-03-04 | Azure Local Cloud Azure Local Cloudnology | Full rewrite to standards — complete frontmatter, 4-tab structure, orchestrated reads infrastructure.yml, standalone with config region, Navigation and Version Control |
