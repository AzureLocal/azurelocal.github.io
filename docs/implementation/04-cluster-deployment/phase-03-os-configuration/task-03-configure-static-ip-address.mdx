...
sidebar_position: 3
description: "Configure static IP addresses on the management network adapter for each Azure Local cluster node using explicit values from infrastructure.yml."
category: Runbook
scope: Network IP configuration
purpose: Assign static management IP addresses to all nodes using explicit IP values from infrastructure.yml
...
> **SCOPE**: Network IP configuration 
> **PURPOSE**: Assign static management IP addresses to all nodes using explicit IP values from `infrastructure.yml` — not read from DHCP 
...
Configure a static IP address on the management network adapter of each Azure Local node. All IP parameters — address, subnet prefix, gateway, and DNS — come explicitly from `infrastructure.yml`. Scripts do **not** read the current DHCP-assigned address.

:::warning Explicit values required — verify before running
All IP values in `infrastructure.yml` must be correct before running. The scripts hard-fail if any placeholder values remain, and permanently lock in whatever IP you specify.
:::
...
| Management IP per node | Static IP to assign | `infrastructure.yml: nodes.<name>.management_ip` |
| Subnet prefix length | e.g. `24` for /24 | `infrastructure.yml: network.management.prefix_length` |
| Default gateway | Management network gateway | `infrastructure.yml: network.management.gateway` |
| DNS servers | Primary and secondary DNS | `infrastructure.yml: dns.primary`, `dns.secondary` |
| Management NIC name | Exact adapter name on all nodes | `infrastructure.yml: cluster.management_nic_name` (from Phase 01 hardware discovery) |
...
The adapter number shown in SConfig varies. Before selecting, run `Get-NetAdapter` in PowerShell to list all adapters and confirm the correct name. The management adapter name is recorded in `infrastructure.yml: cluster.management_nic_name` from the iDRAC discovery performed in Phase 01.
...
2. Run `Get-NetAdapter` in PowerShell — note the adapter name matching `infrastructure.yml: cluster.management_nic_name`
...
8. Enter the static IP address from `infrastructure.yml: nodes.<name>.management_ip`
9. Enter the subnet mask (e.g., `255.255.255.0` for /24, `255.255.255.128` for /25)
10. Enter the default gateway from `infrastructure.yml: network.management.gateway`
11. Select option **2** — Set DNS Servers; enter primary from `infrastructure.yml: dns.primary`
12. Enter secondary DNS from `infrastructure.yml: dns.secondary`
...
Before running, edit the `#region CONFIGURATION` block at the top of the script with values from `infrastructure.yml`:

| Script Variable | infrastructure.yml Path | Example |
|----------------|------------------------|---------|
...
> **When to use**: Standard deployment — reads all configuration from `infrastructure.yml` and configures all nodes via PSRemoting
...
function Get-NodeConfigs {
 ...
 if (-not $mgmtNet.gateway) { throw "network.management.gateway not set in infrastructure.yml" }
 if (-not $mgmtNet.prefix_length) { throw "network.management.prefix_length not set in infrastructure.yml" }
 if (-not $dns.primary) { throw "dns.primary not set in infrastructure.yml" }
 if (-not $dns.secondary) { throw "dns.secondary not set in infrastructure.yml" }
 if (-not $clusterNIC) { throw "cluster.management_nic_name not set in infrastructure.yml" }
...
$nic = "Embedded NIC 1" # replace with cluster.management_nic_name from infrastructure.yml
...
| `Adapter not found` error | NIC name in config doesn't match the node | Run `Get-NetAdapter` on the node; update `cluster.management_nic_name` in `infrastructure.yml` |
...
| Wrong IP locked in permanently | Incorrect value in `infrastructure.yml` | Correct `management_ip` in yml; rerun the script — it will detect mismatch and reconfigure |
| Gateway unreachable after config | Wrong gateway in `infrastructure.yml` | Verify `network.management.gateway`; check upstream switch VLAN |
