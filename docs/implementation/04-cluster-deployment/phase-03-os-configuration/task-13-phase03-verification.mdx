...
| **03/04** | Static IP + DHCP disabled | IP matches `infrastructure.yml`, DHCP = Disabled |
| **05** | DNS servers | Primary and secondary match `infrastructure.yml` |
...
| **10** | Hostname | `COMPUTERNAME` matches `infrastructure.yml` hostname |
...
| `infrastructure.yml` populated | All node IPs, hostnames, network values set |
...
Run from the management server. Reads all expected values from `infrastructure.yml` and verifies each node remotely.
...
|---|---|---|
| `-ConfigPath` | Recommended | Path to `infrastructure.yml`. Auto-detects if omitted |
