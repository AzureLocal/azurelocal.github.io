...
Run identity checks only using the `Invoke-ArcPrerequisites-Orchestrated.ps1` script with `-SkipConnectivity`. Reads SPN, resource group, Key Vault, and Arc Gateway details from `infrastructure.yml`.
...
| Issue | Resolution |
|---|---|
| SPN not found | Verify `identity.service_principal.client_id` in `infrastructure.yml` matches Entra ID app registration |
