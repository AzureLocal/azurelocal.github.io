...
# infrastructure.yml variables (schema v4.0.0):
...
| Tenant ID | From `infrastructure.yml` | `azure_platform.tenant.id` |
| Subscription ID | From `infrastructure.yml` | `azure_platform.subscriptions.lab.id` |
| Resource Group | From `infrastructure.yml` | `compute.azure_local.arc_resource_group` |
| Region | From `infrastructure.yml` | `azure_platform.region` |
...
|---|---|---|
| `-ConfigPath` | *(required)* | Path to infrastructure.yml |
