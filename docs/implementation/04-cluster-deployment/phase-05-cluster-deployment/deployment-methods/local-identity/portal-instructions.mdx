...
Microsoft explicitly states: do not use the built-in `Administrator` account for Local Identity deployments. Create a separate, non-default local account. The account name is configured at `identity.accounts.account_local_admin_username` in `infrastructure.yml`.
...
Run this script directly on each node via RDP or console session. All variables are defined inline — no `infrastructure.yml` or toolkit access required on the node.
...
Run from the **repo root** on the management/jump box. Reads account credentials and node IPs from `infrastructure.yml` and creates the local account on all cluster nodes via PSRemoting.
...
.PARAMETER LocalAdminUsername
 Override the local admin username. Takes precedence over infrastructure.yml.

.EXAMPLE
 .\Invoke-CreateLocalIdentityAccounts-Orchestrated.ps1 -ConfigPath .\configs\infrastructure.yml
 .\Invoke-CreateLocalIdentityAccounts-Orchestrated.ps1 -ConfigPath .\configs\infrastructure.yml -TargetNode iic-01-n01
 .\Invoke-CreateLocalIdentityAccounts-Orchestrated.ps1 -ConfigPath .\configs\infrastructure.yml -WhatIf
...
| **Cluster name** | `<CLUSTER_NAME>` | From `infrastructure.yml: compute.arm_deployment.cluster_name` |
| **Region** | `<AZURE_REGION>` | From `infrastructure.yml: azure_platform.azure_tenants.aztenant_location` |
...
The number of intents, traffic type assignments, and adapter bindings configured here must match the design from your planning and discovery sessions, as captured in `infrastructure.yml` under `compute.arm_deployment.network_intents`. Do not assume a specific number of intents or a fixed layout.
...
For adapter assignments for each intent, refer to `compute.arm_deployment.network_intents[*].intent_adapters` in your `infrastructure.yml`.
:::
...
|---------|-------|--------|
| Starting IP | First available IP | `infrastructure.yml: compute.arm_deployment.ip_allocation.starting_ip` |
| Ending IP | Last available IP | `infrastructure.yml: compute.arm_deployment.ip_allocation.ending_ip` |
| Subnet mask | e.g., `255.255.255.0` | `infrastructure.yml: compute.arm_deployment.subnet_mask` |
| Default gateway | Gateway IP | `infrastructure.yml: compute.arm_deployment.default_gateway` |
| DNS servers | Primary, Secondary | `infrastructure.yml: compute.arm_deployment.dns_servers` |
| **Zone name (domain)** | `<DNS_ZONE_NAME>` | `infrastructure.yml: compute.arm_deployment.domain_fqdn` — **Local Identity only** |
...
2. In the **Key Vault** field, enter or select the Key Vault name (`infrastructure.yml: security.keyvault.kv_azl.kv_azl_name`)
...
Run from the **repo root** on the management/jump box. Reads node IPs and credentials from `infrastructure.yml` and checks cluster health on all nodes via PSRemoting.
...
Self-contained. No `infrastructure.yml` required. Define all variables in `#region CONFIGURATION` before running.
...
Run from the **repo root** on the management/jump box. Reads node IPs and credentials from `infrastructure.yml` and verifies Local Identity configuration on all nodes via PSRemoting.
...
`VAULT_NAME` = `infrastructure.yml: security.keyvault.kv_azl.kv_azl_name`
:::
