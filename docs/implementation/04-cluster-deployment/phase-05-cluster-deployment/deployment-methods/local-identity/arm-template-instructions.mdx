...
Looks up the `Microsoft.AzureStackHCI` resource provider Object ID and writes it directly into `infrastructure.yml` at `cluster_arm_deployment.resource_provider_object_id`. Without `-UpdateConfig`, the script only prints the GUID.

```powershell title="Invoke (look up Object ID and auto-update infrastructure.yml)"
...
```powershell title="Generate the parameter file from infrastructure.yml (optional pre-review)"
...
Single command: reads `infrastructure.yml`, generates the parameter file, patches `deploymentMode` to `Validate`, and runs `New-AzResourceGroupDeployment`. Use `-DeploymentMode Deploy` in Task 4 to deploy after validation passes.
...
Same script as Task 3, invoked with `-DeploymentMode Deploy`. The script reads `infrastructure.yml`, generates the parameter file, patches `deploymentMode` to `Deploy`, and runs `New-AzResourceGroupDeployment`. This is a long-running operation — do not close the terminal.
...
.EXAMPLE
 .\Deploy-AzureLocalCluster.ps1 -ConfigPath .\configs\infrastructure.yml -AuthType LocalIdentity -DeploymentMode Deploy

.EXAMPLE
 .\Deploy-AzureLocalCluster.ps1 -ParametersFile .\azuredeploy.parameters.local-identity.json -DeploymentMode Deploy

.EXAMPLE
 .\Deploy-AzureLocalCluster.ps1 -ConfigPath .\configs\infrastructure.yml -AuthType LocalIdentity -DeploymentMode Deploy -WhatIf
...
Two orchestrated scripts verify health across all nodes via PSRemoting using node IPs and credentials from `infrastructure.yml`. Run both — the first checks cluster and storage health, the second confirms local identity (AD-less) configuration.
