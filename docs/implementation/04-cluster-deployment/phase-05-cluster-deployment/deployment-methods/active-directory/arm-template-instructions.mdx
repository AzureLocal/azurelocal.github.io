...
The Microsoft official ARM template (`azuredeploy.json`, apiVersion `2025-09-15-preview`) accepts **54 parameters**. Azure Local Cloud provides parameter template files and a generation script that populates all 54 from `infrastructure.yml`. See the [Reference](#reference) section at the bottom for the full parameter catalog.
...
1. **`infrastructure.yml` fully populated** — all 13 sections complete, including `cluster_arm_deployment`, `accounts`, `network_intents`, and `compute.cluster_nodes`
...
4. **Arc nodes registered** — Phase 04 complete; `arc_node_resource_ids` populated in `infrastructure.yml`
...
All commands in this runbook run from the **azl-toolkit repo root** on a management/jump box authenticated to Azure. The toolkit scripts expect `infrastructure.yml` at `configs/infrastructure.yml` relative to the repo root.
:::
...
- **Cluster Key Vault** — Created automatically by the ARM template when `createNewKeyVault` is `true`. The name (`keyVaultName`) and other settings are already defined in `infrastructure.yml`.
:::
...
```powershell title="Step 1 — Define variables from infrastructure.yml"
# Replace placeholders with values from your infrastructure.yml
...
**When to use**: You have a populated `infrastructure.yml` and want all AD checks run automatically.
...
**When to use**: No `infrastructure.yml` required — all values are defined inline.
...
The `hciResourceProviderObjectID` parameter requires the tenant-specific Object ID of the `Microsoft.AzureStackHCI` resource provider service principal. This GUID is unique per Entra ID tenant and must be looked up once, then stored in `infrastructure.yml` at `cluster_arm_deployment.resource_provider_object_id`.
...
**Next**: Open `infrastructure.yml`, find `cluster_arm_deployment.resource_provider_object_id`, and paste the GUID as the value.

</TabItem>
<TabItem value="orchestrated" label="Config-Driven Script">

**When to use**: Look up the GUID and automatically write it into `infrastructure.yml`.
...
```powershell title="Look up and auto-update infrastructure.yml"
...
The `-UpdateConfig` flag writes the Object ID directly into `infrastructure.yml` at `cluster_arm_deployment.resource_provider_object_id`. Without it, the script only prints the GUID.
...
**When to use**: You have a populated `infrastructure.yml` and want all 54 parameters mapped automatically. **This is the recommended path.**

```powershell title="Generate the parameter file from infrastructure.yml"
...
1. Reads `infrastructure.yml` and maps all 54 parameters from the YAML config sections
...
# Values from infrastructure.yml
...
2. Reads `infrastructure.yml` for subscription, resource group, cluster name
...
**When to use**: Automated verification across all nodes using `infrastructure.yml`.
