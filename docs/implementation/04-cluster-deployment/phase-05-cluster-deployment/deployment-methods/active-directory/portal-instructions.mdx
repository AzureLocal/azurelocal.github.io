...
<TabItem value="orchestrated" label="Invoke- Orchestrated">

Reads cluster node IPs and credentials from `infrastructure.yml`. Resolves credentials from Key Vault. Logs results to `./logs/`.
...
No `infrastructure.yml` required. Edit the `#region CONFIGURATION` block with node IPs before running.
