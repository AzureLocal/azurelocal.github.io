...
Naming conventions are established during discovery workshops with customers to ensure consistency across all Azure Local Anywhere infrastructure components. Azure Local Cloud follows **Microsoft Cloud Adoption Framework (CAF)** naming standards aligned with Azure best practices and the Azure Local Cloud Master Variable Registry.
...
:::warning Customer Standards Take Precedence
...
- Align with Azure Local Cloud Master Variable Registry
...
- Support automation via variable substitution (`infrastructure.yml`)
...
|---------------|-----------------|--------------|
| `prefix` | Organization short code (chosen by org) | `tp`, `cid001`, `contoso` |
...
The `{prefix}` is whatever the organization chooses — there is no universal default. Azure Local Cloud uses `tp` internally. Customer deployments use the customer ID (e.g., `cid001`). The prefix must be consistent across all management groups in a tenant.
:::
...
```text
Tenant Root Group
└── {prefix}-{tenant}-root (e.g., cid001-prod-root)
 └── sub-{org}-azurelocal-{site} (e.g., sub-cid001-azurelocal-ral)
```

**Customer Example** (Customer ID: `cid001`, Tenant: `prod`):

| **Resource** | **ID / Name** | **Display Name** |
|-------------|---------------|-------------------|
| Management Group | `cid001-prod-root` | `CID001 Root` |
| Subscription | `sub-cid001-azurelocal-ral` | — |
...
**Customer Example** (Prefix: `cid001`, Tenant: `prod`):

```text
Tenant Root Group
└── cid001-prod-root CID001 Root
 ├── cid001-platform-prod CID001 Platform
 │ ├── cid001-platform-identity-prod CID001 Platform - Identity
 │ ├── cid001-platform-security-prod CID001 Platform - Security
 │ ├── cid001-platform-connectivity-prod CID001 Platform - Connectivity
 │ └── cid001-platform-management-prod CID001 Platform - Management
 ├── cid001-landing-zones-prod CID001 Landing Zones
 ├── cid001-sandbox-prod CID001 Sandbox
 └── cid001-decommissioned-prod CID001 Decommissioned
```
...
**Variable Mapping** (from Master Variable Registry):
```yaml
subscriptions:
...
**Standard VLAN Allocation** (Master Variable Registry):
...
**Pattern**: `cid###-<customer_name>`

**Examples**:
- `cid001-acme-corp` (Customer ID 001 - ACME Corp)
- `cid042-globaltech` (Customer ID 042 - GlobalTech)
- `cid100-enterprise-solutions` (Customer ID 100 - Enterprise Solutions)

**Variable Mapping**:
```yaml
gitlab:
 project_name: "cid{{customer_id}}-{{customer_name}}"
```

### 3.2 GitLab Environments
...
For automation via `infrastructure.yml`, use these variable patterns:
...
All variables shown in `{{curly braces}}` are defined in the **Master Variable Registry** and can be populated from `infrastructure.yml` using automation scripts. See [Key Inputs and Variables](../implementation/03-key-inputs-and-variables.mdx) for complete variable reference.
:::
...
- [ ] **Variable Registry Alignment**: Names map to Master Variable Registry variables
...
- [ ] **Documentation**: All naming conventions documented in `infrastructure.yml`
