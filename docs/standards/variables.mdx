...
Every AzureLocal repo uses a single YAML config file — `config/variables.yml` — as input to all automation (Terraform, Bicep, PowerShell, Ansible). The file is not committed; you copy the template and fill it in.
...
```
 ...
 variables.schema.json   alias-policy.json   legacy-compatible-roots.json
       │
       ▼
 variables.example.yml ──copy──▶ variables.yml ──consumed by──▶ Scripts
       │                              │
       ▼                              ▼
...
Open `config/variables.yml` and replace the IIC (Infinite Improbability Corp) example values with your environment's actual values. The file is organized by the same sections as the master registry.
...
Scripts bootstrap automatically — if `variables.yml` is missing, they copy from the template before continuing.
...
| Item | Path |
|------|------|
| Canonical runtime config | `config/variables.yml` |
| Transitional nested path | `config/variables/variables.yml` |
| Primary template | `config/variables.example.yml` |
| Nested template | `config/variables/variables.example.yml` |

Some repos still use the nested path during migration. Scripts must handle both.

### Bootstrap Policy

All config-driven entry-point scripts must implement this sequence:

1. Check for runtime config at the repo's expected path.
2. If missing, locate template: `config/variables.example.yml` → `config/variables/variables.example.yml`.
...
**Key behaviors:**
- Auto-bootstraps from `variables.example.yml` if `variables.yml` is missing
...
All tool-specific parameter files are derived from `config/variables.yml`:
