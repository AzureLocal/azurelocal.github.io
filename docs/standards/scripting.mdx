...
- If runtime config is missing, auto-create it from `variables.example.yml` before parsing
- During migration, support repo-specific nested path (`config/variables/variables.yml`) where required

### Required Bootstrap Sequence

For every config-driven entry point:

1. Resolve repository runtime config path.
2. Check for runtime config file.
3. If missing, locate template:
