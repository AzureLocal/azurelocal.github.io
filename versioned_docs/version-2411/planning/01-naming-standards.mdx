...
**CAF Pattern**: `nsg-<org>-<purpose>`

NSGs control inbound and outbound network traffic for subnets and network interfaces. On Azure Local, NSGs are created via Azure Local SDN and can be associated with logical networks at creation time using the `--network-security-group` parameter.

**Examples**:
- `nsg-iic-management` (Management logical network NSG)
- `nsg-iic-production` (Production workload NSG)
- `nsg-iic-avd` (AVD session host NSG)
- `nsg-iic-sql` (Database server NSG)

**Variable Mapping**:
```yaml
networking:
  nsgs:
    management: "nsg-{{site_code}}-management"
    production: "nsg-{{site_code}}-production"
    avd: "nsg-{{site_code}}-avd"
```
...
| Network Security Group | `nsg` | nsg-iic-management |
